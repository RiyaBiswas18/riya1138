...
def test_create_ticket():
    response = client.post("/ticket", json={
        "id": 1,
        "flight_name": "Usbangla",
        "flight_date": "2025-10-22",
        "flight_time": "10:30",
        "destination": "India"
    })
    assert response.status_code == 200
    assert response.json()["flight_name"] == "Usbangla"
...
def test_update_ticket():
    response = client.put("/ticket/1", json={
        "id": 1,
        "flight_name": "Usbangla a",
        "flight_date": "2025-10-25",
        "flight_time": "16:00",
        "destination": "Turkey"
    })
    assert response.status_code == 200
    assert response.json()["flight_name"] == "Usbangla a"
    assert response.json()["destination"] == "Turkey"
